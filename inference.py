...
IMPORTANT:
    This script assumes you used a scaling factor of SCALE = 1e6
...
CHECKPOINT_DIR = Path("logs/mst")   # <- this matches your tree
...
def main():
    ...
    # Pick an example facility
    example_fac_id = list(facility_dict.keys())[0]
    # Pick an example sector (if any)
    example_sector_name = list(sector_dict.keys())[0] if sector_dict else None

    print(f"\n=== Plotting hierarchy for fac_id={example_fac_id} sector={example_sector_name} ===")
    plot_hierarchy_last_window(
        model,
        fac_id=example_fac_id,
        context_length=T,
        forecast_horizon=H,
